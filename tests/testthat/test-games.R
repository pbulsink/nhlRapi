...
test_that("Game boxscore works", {
  ...
  expect_equal(names(q$data), c("teams", "officials"))

  # Expect Failures
  expect_error(getGameBoxscore(1))
  expect_error(getGameBoxscore("bad"))
  expect_error(getGameBoxscore(c(2018020100,
    2018020101)))
})
...
test_that("Game linescore works", {
  ...
  expect_s3_class(q, 'nhl_api')
  expect_equal(names(q$data), c("currentPeriod",
    "currentPeriodOrdinal", "currentPeriodTimeRemaining",
    "periods", "shootoutInfo", "teams", "powerPlayStrength",
    "hasShootout", "intermissionInfo", "powerPlayInfo"))

  # Expect Failures
  expect_error(getGameLinescore(1))
  expect_error(getGameLinescore("bad"))
  expect_error(getGameLinescore(c(2018020100,
    2018020101)))
})
...
test_that("Game feed works", {
  ...
  expect_s3_class(q, 'nhl_api')
  expect_equal(names(q$data), c("gamePk",
    "link", "metaData", "gameData", "liveData"))
  expect_equal(q$data$link, "/api/v1/game/2018020100/feed/live")
